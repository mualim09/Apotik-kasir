...
<div class="print-faktur">
    ...
    <div class="col-md-12 grid-margin stretch-card">
          <table class="table header">
            ...
            <tr>
              ...
              <td colspan="2" style="width:30%"><h5><strong>Pelanggan</strong><br><small><?=$master->pelanggan?></small></h5></td>
            </tr>
            <tr>
              <td><h5><small><?=$profile->alamat?></small></h5></td>
              <td colspan="2"><h5><small><?=$master->alamat?></small></h5></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td><h5><small>Jenis Pasien</small></h5></td>
              <td><h5><small>: <?=$master->jenis_pasien?></small></h5></td>
            </tr>
            </tbody>
          </table>
        </div><!-- /.col -->
        ...
        <div class="col-md-12 grid-margin">
          <table class="table table-bordered">
            ...
            <tbody>
              ...
              $inc = 0;
              $count = count($detail);
              $sum_hrgjual = $sum_diskon = 0;
                foreach($detail as $data){
                  $inc++;
                  $sum_hrgjual += $data->hrg_jual*$data->jumlah;
                  $sum_diskon  += $data->hrg_jual*$data->diskon/100;
                  ...
                  <tr>
                    ...
                    <td style="text-align:right"><?=number_format($data->hrg_jual)?></td>
                    <td style="text-align:right"><?=number_format($data->diskon)?></td>
                    <td style="text-align:right"><?=number_format($data->hrg_jual*$data->jumlah)?></td>
                  </tr>
                  <?=$margin?>
                  <?php
                }
              ...
              ?>
              <tr>
                <td colspan="5"><strong>Total Harga Jual</strong></td>
                <td class="text-right"><strong><?=number_format($sum_hrgjual)?></strong></td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td class="text-right"><strong><?=number_format($sum_hrgjual-$sum_diskon)?></strong></td>
              </tr>
              <tr>
                <td colspan="5"><strong>PPN = 10% x Dasar Pengenaan Pajak</strong></td>
                <td class="text-right"><strong><?=number_format(($sum_hrgjual-$sum_diskon)*10/100)?></strong></td>
              </tr>
              <tr>
                <td colspan="5"><strong>Jumlah Harus Dibayar</strong></td>
                <td class="text-right"><strong><?=number_format(($sum_hrgjual-$sum_diskon)+(($sum_hrgjual-$sum_diskon)*10/100))?></strong></td>
              </tr>
            </tbody>
          </table>
    </div><!-- /.col -->
  ...
  <div class="row">
    <div class="col-md-12">
      <h5><?='Terbilang '.ucwords(terbilang(($sum_hrgjual-$sum_diskon)+(($sum_hrgjual-$sum_diskon)*10/100))).' Rupiah'?></h5>
    </div>
  </div>
  ...
  <div class="row" style="margin-top:15px">
    <div class="offset-8 col-md-4 grid-margin stretch-card">
          <table class="table header">
            <tbody>
              <tr>
                <td class="text-center">Srono, <?=date('d').' '.bulan(date('m')).' '.date('Y')?></td>
              </tr>
              <tr>
